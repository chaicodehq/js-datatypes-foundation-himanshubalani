...
export function addUrgentItem(cart, item) {
  ...
  if (item.length == 0 || typeof item != "string") return cart;
  const count = cart.unshift(item);

  return cart;
}

export function removeLastItem(cart) {
  if (Array.isArray(cart) == false || cart == []) return undefined;
