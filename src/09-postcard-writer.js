...
export function writePostcard(sender, receiver, message) {
  if ( typeof receiver != "string" || typeof sender != "string" || typeof message != "string" || sender.trim() == '' || receiver.trim() == '' || message.trim() == '' ) return ""
  return `Priy ${receiver},\n\n${message}\n\nAapka/Aapki,\n${sender}`
}

export function isValidPincode(code) {
  const inval = /^\d+$/;
  if (typeof code != "string" || code.startsWith("0") || code.length !== 6 || !inval.test(code) )return false
  return true
}
...
export function countVowels(message) {
  ...
  const matres = message.match(/[aeiouAEIOU]/g);
  if (matres === null) return 0;
  return matres.length
}
